...
public class PDFParametersConstruction {
	...
	private ClassLoader cl = new URLClassLoader(new URL[] { PDFParametersConstruction.class.getClassLoader()
	...
	public Map<String, Object> constructParameters(Invoice invoice) {
		try {
			...
			String billingTemplateName = billingCycle != null && billingCycle.getBillingTemplateName() != null ? billingCycle
			...
			String templateDir = new StringBuilder(resDir).append(File.separator).append(billingTemplateName)
			...
			if (TIP_PAYMENT_METHOD.equals(billingAccount.getPaymentMethod().toString())) {
				...
				if (netToPay.signum() != 1) {
					...
					if (bankCoordinates == null || bankCoordinates.getBankCode() == null) {
						...
						TIP tip = new TIP(provider.getInterBankTitle().getCodeCreancier(), provider.getInterBankTitle()
								.getCodeEtablissementCreancier(), provider.getInterBankTitle().getCodeCentre(),
								bankCoordinatesEmpty, billingAccount.getCustomerAccount().getCode(), invoice.getId(),
								invoice.getInvoiceDate(), invoice.getDueDate(), netToPay);
						parameters.put(PdfGeneratorConstants.HIGH_OPTICAL_LINE_KEY, tip.getLigneOptiqueHaute());
						parameters.put(PdfGeneratorConstants.LOW_OPTICAL_LINE_KEY, tip.getLigneOptiqueBasse());
					} else {
						TIP tip = new TIP(provider.getInterBankTitle().getCodeCreancier(), provider.getInterBankTitle()
								.getCodeEtablissementCreancier(), provider.getInterBankTitle().getCodeCentre(),
								bankCoordinates, billingAccount.getCustomerAccount().getCode(), invoice.getId(),
								invoice.getInvoiceDate(), invoice.getDueDate(), netToPay);
						parameters.put(PdfGeneratorConstants.HIGH_OPTICAL_LINE_KEY, tip.getLigneOptiqueHaute());
						parameters.put(PdfGeneratorConstants.LOW_OPTICAL_LINE_KEY, tip.getLigneOptiqueBasse());
					}
			...
			Map<String, String> baCustomFields = getBACustomFields(billingAccount, provider);
	...
	private Map<String, String> getBACustomFields(BillingAccount billingAccount, Provider provider) {
		List<CustomFieldTemplate> customFieldTemplates = customFieldTemplateService.findByAccountLevel(
				AccountLevelEnum.BA, provider);
			...
			for (CustomFieldTemplate cf : customFieldTemplates) {
				...
				if (cfi != null) {
					if (cf.getFieldType() == CustomFieldTypeEnum.DATE) {
						Date dateField = billingAccount.getInheritedCustomDateValue(cf.getCode());
						if (dateField != null) {
							customFields.put(cf.getCode(), DateUtils.formatDateWithPattern(dateField, "MM-dd-yyyy"));
						}
					} else if (cf.getFieldType() == CustomFieldTypeEnum.DOUBLE) {
						Double doubleField = billingAccount.getInheritedCustomDoubleValue(cf.getCode());
						if (doubleField != null) {
							customFields.put(cf.getCode(), String.valueOf(doubleField));
						}
					} else if (cf.getFieldType() == CustomFieldTypeEnum.LONG) {
						Long longField = billingAccount.getInheritedCustomLongValue(cf.getCode());
						if (longField != null) {
						...
						String stringField = billingAccount.getInheritedCustomStringValue(cf.getCode());
	...
	public String getCustomerAddress(Invoice invoice) {
		...
		if (customerAccount.getName() != null) {
			...
			if (customerAccount.getName().getTitle() != null) {
				name = catMessagesService.getMessageDescription(customerAccount.getName().getTitle(),
						billingAccountLanguage) + " ";
			}
