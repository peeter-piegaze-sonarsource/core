	...
	public int importSubscription(CheckedSubscription checkSubscription,
		...
		for (org.meveo.model.jaxb.subscription.ServiceInstance serviceInst : checkSubscription.serviceInsts) {
			try {
				...
				if (serviceInst.getSubscriptionCharges() != null
						&& serviceInstance.getSubscriptionChargeInstances() != null) {
					updateCharges(serviceInstance.getSubscriptionChargeInstances(),
							serviceInst.getSubscriptionCharges());
				}
				if (serviceInst.getTerminationCharges() != null
						&& serviceInstance.getTerminationChargeInstances() != null) {
			...
			for (Charge chargeToTest : charges) {
				...
				if (charge != null) {
					...
					if (charge.getAmountWithTax() != null) {
						chargeInstance.setAmountWithTax(new BigDecimal(charge.getAmountWithTax().replace(',', '.'))
