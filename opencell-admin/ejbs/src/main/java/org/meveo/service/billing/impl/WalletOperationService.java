...
public class WalletOperationService extends BusinessService<WalletOperation> {
    ...
    private void cancelWOAndRatedTransactionNotBilled(RecurringChargeInstance chargeInstance, Boolean isApplyInAdvance) {
        ...
        if (isApplyInAdvance) {
             terminationDate = DateUtils.addSecondsToDate(chargeInstance.getTerminationDate(), -1);
        }
        getEntityManager().createNamedQuery("WalletOperation.cancel").setParameter("chargeInstance", chargeInstance).setParameter("terminationDate", terminationDate)
                .executeUpdate();
    ...
    public List<WalletOperation> applyReccuringCharge(RecurringChargeInstance chargeInstance, boolean reimbursement, RecurringChargeTemplate recurringChargeTemplate,
        ...
        while (applyChargeToDate != null && applyChargeOnDate.getTime() < applyChargeToDate.getTime()) {
            ...
            if (reimbursement) {
                ...
                createNewWO = isChargeAlreadyBilled(chargeInstance, applyChargeOnDate, nextChargeDate, true);
            }
