...
public class GenericPagingAndFilteringUtils {
    ...
    private static final String GENERIC_FIELDS = "genericFields";
    ...
    constructImmutableGenericPagingAndFiltering(MultivaluedMap<String, String> queryParams) throws JsonProcessingException {
        ...
        while (itQueryParams.hasNext()){
            ...
            else if ( aKey.equals( GENERIC_FIELDS ) ) {
                ObjectMapper jsonMapper = new ObjectMapper();
                if ( aList.get(0).charAt(0) == OPEN_HOOK ) {
                    Set genericFields = jsonMapper.readValue( aList.get(0), Set.class );
                    builder.addAllGenericFields(genericFields);
                }
            }
            else {
                ObjectMapper jsonMapper = new ObjectMapper();
                if ( aList.get(0).charAt(0) == OPEN_HOOK ) {
                    List listElements = jsonMapper.readValue( aList.get(0), List.class );
