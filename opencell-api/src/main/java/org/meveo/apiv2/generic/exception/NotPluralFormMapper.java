 ...
 * @author Thang Nguyen
 **/
public class NotPluralFormMapper implements ExceptionMapper<NotPluralFormException>  {
    private final Logger log = LoggerFactory.getLogger(this.getClass());
    private final ExceptionSerializer exceptionSerializer = new ExceptionSerializer(Response.Status.BAD_REQUEST);

    @Override
    public Response toResponse(NotPluralFormException exception) {
        log.error("A plural form exception occurred ", exception);
