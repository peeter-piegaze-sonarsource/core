...
@Stateless
@Named
public class LangService extends PersistenceService<Lang> {

    public Lang findByCode(String code) {
        if (code == null) {
            return null;
        }
        QueryBuilder qb = new QueryBuilder(Lang.class, "c");
        qb.addCriterion("languageCode", "=", code, false);

        try {
            return (Lang) qb.getQuery(getEntityManager()).getSingleResult();
